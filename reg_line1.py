...
class RegLine:
    ...
    def reg_line(self, img, show=False):
        ...
        # warped = cv2.resize(warped, (80, 50))
        img_to = warped.copy()
        pt = time.time()
        # warped = 
        ...
        if (p_s > 0):
            ...
            cv2.circle(out_img,(int(self.points[p_s-1][0]),int(self.points[p_s-1][1])),2,(0,255,255),1)
            err2 = self.img_size[1]//2-(self.points[p_s-1][0]+self.points[qq-1][0])/2
            err = self.points[p_s-1][0]-self.points[qq-1][0]
            xxx1 = self.img_size[1]//2 - self.points[p_s-1][0]
            xxx2 = self.img_size[1]//2 - self.points[qq-1][0]
        # if show==True:
        ...
        if show==True:
            cv2.imshow("CenterLine",out_img)
        return err/(self.img_size[1]/2), err2/(self.img_size[1]/2), out_img, su2, img_to, xxx1/(self.img_size[1]/2), xxx2/(self.img_size[1]/2), pt